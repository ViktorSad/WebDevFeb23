...
Netflix.prototype.addMovie = function (
  title,
  genres = [],
  description,
  director,
...
Netflix.prototype.findMoviesByCategory = function (cat) {
  ...
  return this.movies.filter((movie, i) => {
    // here we are retrive movie object in filter method
    let categoryArr = movie.genres; // subarray of each movie object
    return categoryArr.filter((item) => {
      // return final filter result by category name of a movie
...
Netflix.prototype.findMoviesBy = function (prop, userinput) {
  ...
  return this.movies.filter((movie) =>
    String(movie[prop]).toLowerCase().includes(userinput.toLowerCase())
  );
...
Netflix.prototype.findMovieByTitleAndDelete = function (title) {
  if (title !== undefined) {
    // check if title is defined? yes

    // then use title to find object position number in array
    let findMovieIndex = this.movies.findIndex(
      (movie) => movie.title.toLowerCase() === title.toLowerCase()
    ); // get position number of the object in the array list

    if (findMovieIndex !== -1) {
      // title is defined and check if findMovieindex is correct
